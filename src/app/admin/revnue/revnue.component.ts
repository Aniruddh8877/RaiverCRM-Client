...
export class RevnueComponent {
 ...
 onStaffSelected() {
  if (!this.LeadAssign.StaffId) {
    this.toastr.error("Please select a staff member");
    return;
  }
  this.getLeadDetailsList();
}
  ...
  getLeadDetailsList() {
    ...
    this.service.getLeaveDetailRevenueList(obj).subscribe(
      (r1: any) => {
        let response = r1;
        if (response.Message === ConstantData.SuccessMessage) {
          this.LeadDetailList = response.LeadDetailRevenueList;
          this.Sum = response.Sum;
          console.log("Fetched Lead Detail List:", this.LeadDetailList);
          console.log("Fetched Lead Det2345324ail List:", this.Sum);
        } else {
          this.toastr.error(response.Message);
        }
        this.dataLoading = false;
  ...
  SaveLeadDetail() {
    ...
    this.service.SaveLeadDetail(payload).subscribe((response: any) => {
      if (response.Message === ConstantData.SuccessMessage) {
        ...
        this.getLeadDetailsList(); // Refresh table
        this.resetForm();
      } else {
        this.toastr.error(response.Message);
      }
